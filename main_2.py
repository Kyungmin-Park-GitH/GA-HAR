"""Entry point for running the NSGA-II optimisation on the HAR datasets (2 sensors)."""
from __future__ import annotations

import argparse
from typing import List

from src.data_utils_2 import load_dataset_2sensors

try:
    from src.ga_2 import NSGA2
except ModuleNotFoundError as exc:
    if getattr(exc, "name", None) == "src.ga_2":
        raise ModuleNotFoundError(
            "`src.ga_2` 모듈을 찾을 수 없습니다. 사용하려는 아키텍처의 GA 파일을 "
            "`src/ga_2.py`로 이름을 바꾼 후 다시 실행해 주세요. 예: `ga_lstm_2.py` → `ga_2.py`."
        ) from exc
    raise


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimise 2-sensor HAR models with NSGA-II")
    parser.add_argument(
...
def main() -> None:
    ...
    coventry, infra = load_dataset_2sensors(
