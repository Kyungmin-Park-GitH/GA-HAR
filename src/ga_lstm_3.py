"""Multi-objective genetic algorithm (NSGA-II) for 3-sensor LSTM HAR model search."""
...
from .training_lstm_3 import EvaluationMetrics, TrainingConfig, evaluate_with_kfold
...
class Evaluator:
    ...
    def evaluate(self, genome: Genome, generation: int, individual_index: int, total_individuals: int) -> Tuple[Dict[str, EvaluationMetrics], Dict[str, Tuple[EvaluationMetrics, ...]]]:
        config = TrainingConfig(learning_rate=genome.learning_rate, batch_size=genome.batch_size, lstm_layers=genome.lstm_layers, units=genome.units, dropout_rate=genome.dropout_rate)
