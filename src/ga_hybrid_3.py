"""Multi-objective genetic algorithm (NSGA-II) for 3-sensor hybrid CNN-LSTM HAR model search."""
...
from .training_hybrid_3 import EvaluationMetrics, TrainingConfig, evaluate_with_kfold
...
def _compute_objectives(fold_metrics_by_dataset: Dict[str, Tuple[EvaluationMetrics, ...]]) -> Tuple[float, float, float, float]:
    if not fold_metrics_by_dataset: return 0.0, 0.0, 0.0, 0.0
    metrics_sequences = list(fold_metrics_by_dataset.values())
    fold_count = len(metrics_sequences[0]) if metrics_sequences else 0
    if fold_count == 0: return 0.0, 0.0, 0.0, 0.0
    for metrics in metrics_sequences:
        if len(metrics) != fold_count: raise ValueError("All datasets must provide the same number of folds")
    mean_acc, min_acc, mean_f1, min_f1 = [], [], [], []
    for fi in range(fold_count):
        accs = [m[fi].accuracy for m in metrics_sequences]
        f1s = [m[fi].f1 for m in metrics_sequences]
...
class Genome:
    ...
    def crossover(self, other: "Genome") -> "Genome":
        return Genome(**{f: getattr(self if random.random() < 0.5 else other, f) for f in self.__dataclass_fields__})
...
class EvaluationLogger:
    ...
    def _append_row(path: Path, row: Sequence[object]) -> None:
        with path.open("a", newline="", encoding="utf-8") as f: csv.writer(f).writerow(row)
    ...
    def log_generation(self, generation: int, individuals: Sequence[Individual]) -> None:
        ...
        if not self.overall_file.exists():
            ...
            for d in self.datasets: h.extend([f"{self.dataset_slugs[d.name]}_{x}" for x in ["accuracy", "recall", "precision", "f1_score"]])
...
class Evaluator:
    ...
    def evaluate(self, genome: Genome, generation: int, individual_index: int, total_individuals: int) -> Tuple[Dict[str, EvaluationMetrics], Dict[str, Tuple[EvaluationMetrics, ...]]]:
        config = TrainingConfig(learning_rate=genome.learning_rate, batch_size=genome.batch_size, lstm_layers=genome.lstm_layers, units=genome.units, dropout_rate=genome.dropout_rate, conv_layers=genome.conv_layers, filters=genome.filters, kernel_size=genome.kernel_size)
...
def _select_tournament(population: Sequence[Individual], tournament_size: int = 3) -> Individual:
    ...
    return sorted(random.sample(population, ts), key=lambda i: (i.rank, -i.crowding_distance))[0]
