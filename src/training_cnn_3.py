"""Training utilities for evaluating genomes on the HAR datasets with 3 sensors (CNN)."""
...
from .model_cnn_3 import HARConvNet3Sensor
...
class EvaluationMetrics:
    ...
    def average(metrics: Sequence["EvaluationMetrics"]) -> "EvaluationMetrics":
        ...
        return EvaluationMetrics(
            accuracy=accuracy, recall=recall, precision=precision, f1=f1, confusion_matrix=confusion
        )
...
def train_with_validation(
    ...
    model = HARConvNet3Sensor(
        num_classes=num_classes, conv_layers=config.conv_layers, filters=config.filters,
