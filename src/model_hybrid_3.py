"""Hybrid CNN-LSTM network definition for the infrared HAR task with 3 sensor inputs."""
...
class HARHybridNet3Sensor(nn.Module):
    """CNN-LSTM hybrid for 3-sensor 8x8 infrared inputs with late fusion."""
    ...
    def __init__(
        ...
        for _ in range(conv_layers):
            ...
            conv_blocks3.append(nn.ReLU())
            in_channels = filters
        self.conv3 = nn.Sequential(*conv_blocks3)
        ...
        # Classifier takes concatenated features from all 3 sensors
        self.classifier = nn.Linear(units * 3, num_classes)

    def forward(
        self, input1: torch.Tensor, input2: torch.Tensor, input3: torch.Tensor
    ) -> torch.Tensor:
        """Runs a forward pass with 3 sensor inputs.

        Args:
            input1: Tensor with shape ``(batch_size, 40, 8, 8)`` for sensor 1.
            input2: Tensor with shape ``(batch_size, 40, 8, 8)`` for sensor 2.
            input3: Tensor with shape ``(batch_size, 40, 8, 8)`` for sensor 3.
        """
        ...
        # Process sensor 3
        frames3 = input3.view(batch_size * sequence_length, 1, height, width)
        conv_features3 = self.conv3(frames3)
        conv_features3 = self.frame_dropout(conv_features3)
        feature_vectors3 = conv_features3.view(batch_size, sequence_length, -1)
        _, (hidden3, _) = self.lstm3(feature_vectors3)
        final_state3 = hidden3[-1]

        # Combine and classify
        combined = torch.cat([final_state1, final_state2, final_state3], dim=1)
