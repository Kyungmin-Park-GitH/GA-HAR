"""LSTM-based network definition for the infrared HAR task with 3 sensor inputs."""
...
class HARLSTMNet3Sensor(nn.Module):
    """Stacked LSTM classifier for 3-sensor 8x8 infrared sequences with late fusion."""
    ...
    def __init__(
        ...
        for layer_index in range(lstm_layers):
            ...
            input_size = units
        self.lstm_stack3 = nn.Sequential(*layers3)

        # Classifier takes concatenated features from all 3 sensors
        self.classifier = nn.Sequential(
            nn.Linear(units * 3, num_classes),
            nn.Softmax(dim=1),
        )
    ...
    def forward(
        ...
        # Process sensor 3
        seq3 = input3.view(batch_size, self.sequence_length, self.feature_size)
        out3 = self.lstm_stack3(seq3)
        if out3.dim() == 3:
            out3 = out3[:, -1, :]
