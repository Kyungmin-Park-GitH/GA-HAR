"""Utility functions for loading and preparing multi-sensor HAR infrared datasets (2 sensors)."""
...
@dataclass
class DatasetSplit:
    """Container storing preprocessed samples and labels for a dataset split (2 sensors)."""
...
@dataclass
class DatasetInfo:
    """Stores all the information required to train on a dataset (2 sensors)."""
...
def load_dataset_2sensors(
    ...
    """Loads datasets for 2-sensor training.
    ...
    - coventry (sensor1, sensor2)
    - infra (sensor3, sensor4)

    Returns:
        Tuple of (coventry_dataset, infra_dataset) with 2-sensor structure.
    """
    ...
    assert np.array_equal(cov_s1_train_labels, cov_s2_train_labels), "Coventry train labels mismatch"
    assert np.array_equal(cov_s1_test_labels, cov_s2_test_labels), "Coventry test labels mismatch"
    assert np.array_equal(infra_s3_train_labels, infra_s4_train_labels), "Infra train labels mismatch"
    assert np.array_equal(infra_s3_test_labels, infra_s4_test_labels), "Infra test labels mismatch"
    ...
    # Create coventry test dataset (2 sensors)
    ...
    # Create infra test dataset (2 sensors)
