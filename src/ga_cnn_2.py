"""Multi-objective genetic algorithm (NSGA-II) for 2-sensor CNN HAR model search."""
...
from .data_utils_2 import DatasetInfo
from .training_cnn_2 import EvaluationMetrics, TrainingConfig, evaluate_with_kfold
...
def _compute_objectives(
    ...
    """Computes average/min accuracy and F1 objectives from fold metrics."""
...
class Evaluator:
    """Handles the evaluation of genomes on the available datasets (2 sensors)."""
    ...
    def evaluate(
        ...
        # Use training data from first dataset (both have same training data)
        ...
        test_sets = {
            dataset.name: (
                dataset.test.samples_sensor1,
                dataset.test.samples_sensor2,
                dataset.test.labels,
            )
            for dataset in self.datasets
        }
        ...
        fold_metrics_by_dataset = evaluate_with_kfold(
            config=config,
            inputs1=train_inputs1,
            inputs2=train_inputs2,
            labels=train_labels,
            test_sets=test_sets,
...
class NSGA2:
    """Implementation of the NSGA-II algorithm for hyperparameter optimisation (2 sensors)."""
